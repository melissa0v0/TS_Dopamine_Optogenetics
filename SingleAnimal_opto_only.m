function [M_plot, Response, ste_Response] = SingleAnimal_opto_3frequency(mouse_name)
%% initialize

close all
d = load('4_opto_only_D1_response.mat');
dat = d.signal_raw;
...
temp_A = trial_type_sig_on(i,1);    
    for f = i+1:size(trial_type_sig_on,1)
        if trial_type_sig_on(f,1) < temp_A + 6000  %identify multiple signals within 1s
        temp_B = temp_B + 1;  
        end
    end
...
for i = 1:length(trigger)
    ...
    response = deltaF(:,2000:2500); %2000 is trigger1
